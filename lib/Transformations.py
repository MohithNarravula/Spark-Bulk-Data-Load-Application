...
def create_title_struct(col_name, type_label):
    """
    Reusable helper to create a single title struct if the column is not null.
    """
    return when(col(col_name).isNotNull(),
                struct(lit(type_label).alias("contractTitleLineType"),
                       col(col_name).alias("contractTitleLine")))


def get_contract(df):
    # Pass column names and labels as variables
    titles = array(
        create_title_struct("legal_title_1", "lgl_ttl_ln_1"),
        create_title_struct("legal_title_2", "lgl_ttl_ln_2")
    )

    # Return the array after cleaning out the nulls
    # The Column Method
    # Spark treats x as a column object and checks if that specific struct inside the array exists.
    contract_title_nl = filter(titles, lambda x: x.isNotNull())

    # The Function Style
    # This works exactly the same, but it must ensure that isnull is imported from pyspark.sql.functions
    # contract_title_nl = filter(titles, lambda x: ~isnull(x))
